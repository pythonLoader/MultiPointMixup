...
def train(args, io):
    if args.data == 'MN40':
        ...
        train_loader = DataLoader(ScanObjectNN(partition='train', num_points=args.num_points, aug=args.aug, ver="easy"), num_workers=4,
                                batch_size=args.batch_size, shuffle=True, drop_last=True)
        test_loader = DataLoader(ScanObjectNN(partition='test', num_points=args.num_points, aug=args.aug, ver="easy"), num_workers=4,
                                batch_size=args.test_batch_size, shuffle=True, drop_last=False)
        num_class =15
    elif args.data == 'SONN_HARD':
        train_loader = DataLoader(ScanObjectNN(partition='train', num_points=args.num_points, aug=args.aug, ver="hard"), num_workers=4,
                                batch_size=args.batch_size, shuffle=True, drop_last=True)
        test_loader = DataLoader(ScanObjectNN(partition='test', num_points=args.num_points, aug=args.aug, ver="hard"), num_workers=4,
                                batch_size=args.test_batch_size, shuffle=True, drop_last=False)
        num_class =15
    ...
    if args.wandb:
        wandb.init(project="NIPS2022", name=args.exp_name + datetime.now().strftime('-%Y/%m/%d_%H:%M'))
    ...
    for epoch in range(args.epochs):
        ...
        for data, label in (pbar := tqdm(train_loader)):
            ...
            logits, label = model(data.permute(0,2,1), label, mixup=True, saliency=saliency)
        ...
        if args.scheduler == 'cos':
            ...
            if opt.param_groups[0]['lr'] < 1e-5:
                for param_group in opt.param_groups:
                    param_group['lr'] = 1e-5
        pbar.set_description("Processing %.4f" % opt.param_groups[0]['lr'])
        ...
        for data, label in tqdm(test_loader):
            ...
            batch_size = data.size()[0]
            with torch.no_grad():
                logits, _ = model(data, mixup=False)
            # loss = criterion(logits, label)
            preds = logits.max(dim=1)[1]
            count += batch_size
...
if __name__ == "__main__":
    ...
    parser.add_argument("--manimix", action='store_true') 
    ...
    parser.add_argument('--sample_ver', type=int, default=0) 
    
    args = parser.parse_args()
    _init_()
    if args.no_saliency != True :
        assert args.kermix == True , "argument kermix should be True, if saliency map is used"
    if args.manimix:
        assert args.kermix == True , "argument kermix should be True, if argument manimix is True"
