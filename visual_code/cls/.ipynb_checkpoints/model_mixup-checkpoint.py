    ...
    def mixup(self, xyz, label, x=None, saliency=None):
        ...
        #weighted sampling
            ...
            #max_to_max, distance based re-weighting
                ...
                saliency_perm = saliency_perm * dist**self.temperature2
                saliency_perm = saliency_perm/saliency_perm.sum(-1, keepdim=True)
        ...
        # weight_ori = saliency *ker_weight_ori * alpha 
        # weight_perm = saliency_perm * ker_weight_perm * (1-alpha)
...
class PointNet(nn.Module):
    def __init__(self, args, output_channels=40):
        ...
        self.manimix=args.manimix
        
        if self.kermix:
            self.namemix = namemix(args, output_channels)

    def forward(self, x, label=None, mixup = False, saliency=None, get_mix=False):
        
        xyz = x.permute(0,2,1)
        
        #input mix
        if self.kermix and (self.manimix==False) and mixup:
           x, label, temp = self.namemix.mixup(xyz, label, saliency=saliency)
        
        # if mixup:
        #     x, label = self.base_mixup(xyz, label)
        ...
        #manifold mixup
        if self.kermix and self.manimix and mixup:
            x, label = self.mixup(xyz, label, x)
...
class DGCNN(nn.Module):
    ...
    def forward(self, x, label=None, mixup = False, saliency=None, get_mix=False):
        ...
        # if mixup:    
        #     x, label = self.namemix.base_mixup(xyz, label)
...
class Pointnet2_MSG(nn.Module):
    ...
    def mixup(self, xyz, label, x=None, saliency=None):
        ...
        #max
        elif self.saliency_ver == 2:
            anc_idx = saliency.max(1)[1]
            anchor_ori = xyz[torch.arange(batch_size),anc_idx]
            anchor_perm = anchor_ori[idxs]

        if self.beta is not None:
            alpha = self.beta.sample((batch_size,)).cuda()
        else:
            alpha = 0.5
        
        sub_ori = xyz - anchor_ori[:,None,:]
        sub_ori = ((sub_ori) ** 2).sum(2).sqrt()
        weight_ori = torch.exp(-0.5 * (sub_ori ** 2) / (self.sigma ** 2))  #(M,N)
        weight_ori = weight_ori * alpha

        sub_perm = perm_new - anchor_perm[:,None,:]
        sub_perm = ((sub_perm) ** 2).sum(2).sqrt()   
        weight_perm = torch.exp(-0.5 * (sub_perm ** 2) / (self.sigma ** 2))  #(M,N)
        ...
        return x, label, {"mix" : x, "perm" : perm, "weight":weight}
